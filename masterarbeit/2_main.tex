...
\ForAll{sg in $component$.asset.securityGoals} 
	\State $L_{sg}$.add(sg)	
\EndFor
\State checkConnections($component$) 
\State findAncestors($component$) 
\State findChildren($component$) 
\State filterDuplicates($L_{sg}$)
\State checkSecurityGoalCorrectness($component$, $L_{sg}$)
...
\State add\_sg($cmp.id$, $component$)
\EndFor
\EndIf
...
\Function{checkConnections}{$component$}\\

\EndFunction

\Function{filterDuplicates}{$L_{sg}$}\\

\EndFunction

\Function{checkSecurityGoalCorrectness}{$L_{sg}$}\\
