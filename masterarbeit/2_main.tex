...
\subsubsection{Propagation of security attributes between different abstraction layers}
\label{subsubsec:propagation}

The gathering of information has been briefly mentioned when addressing the overall goal in Section \ref{subsec:goal}. In this section the propagation of security attributes through the respective abstraction layers will be discussed. Here we will look into the dependencies between components and their security properties during transformations.
...
This feature amongst others will be discussed in Section \ref{sec:conclusion} as possible enhancements of future versions. 
...
\Function{addSGAtoC}{$anc$, $child$} \label{line:anc_child} \Comment{ancestor to child} 
...
\Function{addSGCtoA}{$child$, $anc$} \label{line:child_anc} \Comment{child to ancestor}
...
\begin{algorithm}[H] 
\begin{algorithmic}
\Function{controlAggregation}{$L_{sg}$}
...
EMF allows users to create a dynamic instance that is conform to the defined metamodel. Here the example from previous chapters has been chosen. The initial elements have been created accordingly.
...
\begin{algorithm}[H]
\begin{algorithmic}
\Function{securityConceptValidation}{$oldSecurityConcept$, $newSecurityConcept$, $componentsOfInterest$}
\State Boolean $valid$ = true
\ForAll {$comp$ in $componentsOfInterest$}
...
\EndIf
\EndFor
\ForAll {t : oldThreatList}
\If {!threatInList($newThreatList$, $t$)}
\State $valid$ = false
\State return $valid$
\EndIf
\EndFor

\ForAll {c : oldControls}
\If {!controlInList($newControls$, $c$)}
\State $valid$ = false
\State return $valid$
\EndIf
\EndFor
...
The transformation rules have also been implemented in a framework using EMF thus providing a working, scalable solution.  
