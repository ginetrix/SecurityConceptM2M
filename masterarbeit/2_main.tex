...
\ForAll{sg in $component$.asset.securityGoals} 
	\State $L_{sg}$.add(sg)	
\EndFor
...
\State checkConnectionCorrectness()
...
\ForAll{cmp in $component$.SubComponents}
\If{cmp in $L.c$}
\State{$S_{subc}$.add(cmp)} 
\State{findChildren(cmp)}
\State{fixConnection($component$, $cmp$)}
\EndIf
\State $L_{sg}$.add(cmp.asset.securityGoals)
\State fixConnection($component$, $cmp$)
\EndFor
\Else
...
\Function{add\_sg\_anc}{$anc$, $child$} \Comment{ancestor to child}
...
\Function{add\_sg\_child}{$child$, $anc$} \Comment{child to ancestor}
\ForAll {sg in $child$.asset.securityGoals}
\If{sg.securityGoalClass == 'Confidentiality'}
...
\begin{figure}[p]
    \vspace*{-2cm}
    \makebox[\linewidth]{
        \includegraphics[width=1.2\linewidth]{pictures/sg_transformation}
    }
