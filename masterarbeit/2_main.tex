...
Each asset can be viewed separately when looking at security goals and threats. For SGs we can define three separate sets, $SG_C(asset)$, $SG_I(asset)$ and $SG_A(asset)$ where each one contains the respective SGs with either confidentiality, integrity or availability as the security goal class. It follows:

\begin{theorem}
$SG_{CIA}(asset) = SG_C(asset) \cup SG_I(asset) \cup SG_A(asset)$
\end{theorem}

Furthermore we define that for every component $C$ the SG sets are the union of the SGs of its sub-components with the same security goal class. For the confidentiality we have to consider pairs of components and data, since confidentiality is only defined on data. Therefore when looking at a component $c_i$ and a data asset $asset_{d_{j}}$ we can say that the confidentiality of data $d_j$ for component $c_i$ is the union of the SGs of its subcomponents also covering $d_j$. $sgc$ is either confidentiality ($C$), integrity ($I$) or availability ($A$).
...
\begin{theorem} 
$SG_{sgc}(asset_{c_{i}}) = SG_{sgc}(asset_{c_{i_{1}}}) \cup ... \cup SG_{sgc} (asset_{c_{i_{n}}}) $
\end{theorem}

Threats can be defined accordingly. For a threat $T$ with the threat class $tc$ the following union set can be defined. Once again we differentiate between data assets and non-data assets.

\begin{theorem} 
$T_{tc}(c_i, asset_{d_{j}}) = T_{tc}(c_{i_{1}}, asset_{d_{j}}) \cup ... \cup T_{tc} (c_{i_{n}}, asset_{d_{j}}) $
\end{theorem}

The set of threats for non-data assets can be defined accordingly.

\begin{theorem} 
$T_{tc}(asset_{c_{i}}) = T_{tc}(asset_{c_{i_{1}}}) \cup ... \cup T_{tc} (asset_{c_{i_{n}}}) $
\end{theorem}

These were defined to show the transformation steps as well to easy the validation of the model transformation in Section \ref{subsec:validation}.
